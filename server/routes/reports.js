...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      ...
      // Report is still saved, but ambulance wasn't notified
    }
    ...
    res.status(201).json({
      ...
      reportId: report._id,
      ambulanceNotified: report.ambulanceNotified,
    });
  } catch (error) {
...
router.get('/', async (req, res) => {
  try {
    ...
    const reports = await Report.find()
      ...
      .skip(skip)
      .limit(limit)
      .select('-incidentPhoto -faceVerificationPhoto'); // Exclude photos for list view
